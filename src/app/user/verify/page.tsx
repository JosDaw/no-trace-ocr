  ...
  return (
    <Container mx="auto" mt="xl">
      ...
      {(verificationStatus === 'verified' || verificationStatus === 'error') && (
        <Box style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }} pt={25}>
          {verificationStatus === 'verified' && <><IconMoodHappyFilled size={60} color="green" /><Title my={20} ta="center" c="green">Your email has been successfully verified!</Title></>}
          {verificationStatus === 'error' && <><IconMoodSadFilled size={60} color="red" /><Title my={20} ta="center" c="red">Oh no, the verification failed!</Title></>}
          <Flex gap={6}>
            <Link href="/" passHref>
              <Button mt="md" color="red">Go to Home</Button>
            </Link>
            <Link href="/contact" passHref>
              <Button mt="md" color="blue">Contact Us</Button>
            </Link>
          </Flex>
        </Box>
      )}
